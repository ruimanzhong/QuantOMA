...
def verification_steps() -> list[Step]:
    py = sys.executable
    return [
        Step("Check A-share ETF data coverage", [py, "scripts/check_data_coverage.py"]),
        Step("Build local ETF Qlib provider", [py, "scripts/build_etf_qlib_provider.py"]),
        Step(
            "Inspect local ETF Qlib provider",
            [py, "scripts/inspect_qlib_provider.py", "--feature-set", "alpha158_etf"],
        ),
        Step(
            "Build ETF Alpha158 features",
            [py, "scripts/build_qlib_alpha158_features.py", "--feature-set", "alpha158_etf"],
        ),
        Step(
            "Diagnose ETF Alpha158 features",
            [py, "scripts/diagnose_alpha158_features.py", "--feature-set", "alpha158_etf"],
        ),
    ]
