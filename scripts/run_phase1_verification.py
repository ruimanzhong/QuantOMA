...
def verification_steps() -> list[Step]:
    ...
    return [
        ...
        Step("Fetch A-share ETF data", [py, "scripts/fetch_a_share_data.py"], skip_when="skip_fetch"),
        Step("Check A-share data coverage", [py, "scripts/check_data_coverage.py"]),
        Step("Run rule baselines", [py, "scripts/run_rule_baselines.py"]),
        Step("Evaluate rule baselines", [py, "scripts/evaluate_rule_baselines.py"]),
        Step("Diagnose sample attrition", [py, "scripts/diagnose_sample_attrition.py"]),
        Step("Diagnose annual rule performance", [py, "scripts/diagnose_rule_annual_performance.py"]),
        Step("Diagnose subperiod rule performance", [py, "scripts/diagnose_rule_subperiod_performance.py"]),
        Step("Run rule parameter sensitivity", [py, "scripts/run_rule_parameter_sensitivity.py"]),
        Step("Run transaction cost sensitivity", [py, "scripts/run_transaction_cost_sensitivity.py"]),
        Step("Summarize rule robustness", [py, "scripts/summarize_rule_robustness.py"]),
    ]
